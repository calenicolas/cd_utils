...
unixSocketServer.on('data', (data) => {
    ...
    const parameters = [gitRepo, imageName, containerName, servicePort, containerNetwork, proxyContainerName, proxyContainerNetwork, protocol];
    const stringArguments = parameters.join(" ");

    exec("deploy " + stringArguments, () => {});
});